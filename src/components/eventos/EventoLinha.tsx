...
export function EventoLinha({ evento, viewMode, destacar }: Props) {
  ...
  // Professor: card com mais respiro
  return (
    <article className={`border-l-4 ${cfg.borderClass} bg-card rounded-md shadow-sm border border-border p-4 transition-shadow hover:shadow-md ${destacar ? 'ring-2 ring-warning' : ''}`}>
      ...
      {evento.corpo && (
        <p className="text-[13px] leading-relaxed whitespace-pre-line text-foreground mb-2">{evento.corpo}</p>
      )}
      {evento.documentos.length > 0 && (
